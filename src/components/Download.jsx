...
import {
    ...
    WifiOff,
    ShieldCheck,
    Terminal // Swapped for a better Linux icon
} from "lucide-react";
    ...
    const versions = [
        {
            ...
            link: "/pixelsuite.exe", // Corrected path
            button: "Download",
            available: true
        },
        {
        ...
        <div className="min-h-screen bg-slate-50 text-slate-800 font-sans">
            ...
            <section className="py-20 text-center px-6">
                ...
                <p className="text-lg text-slate-600 max-w-2xl mx-auto mb-10">
                    Experience the full power of Pixel Suite offline with native performance 
                    ...
                    {versions.map((v) => (
                        <div key={v.os} className="bg-white rounded-2xl shadow-lg border border-slate-100 overflow-hidden hover:shadow-xl transition-shadow flex flex-col">
                            <div className="p-8 text-center flex-1">
                                <div className={`w-16 h-16 mx-auto rounded-2xl ${v.available ? v.color : 'bg-slate-300'} text-white flex items-center justify-center mb-6 shadow-md`}>
                                    {v.icon}
                                </div>
                                ...
                                {v.available ? (
                                    <a
                                        href={v.link}
                                        download
                                        className={`w-full block py-3 rounded-xl font-bold text-white transition-all ${v.color} ${v.hover} shadow-lg shadow-indigo-500/10 flex items-center justify-center gap-2`}
                                    >
                                        <Download size={18} />
                                        {v.button} 
                                    </a>
                                    ...
                                    <button
                                        disabled
                                        className="w-full py-3 rounded-xl font-bold text-slate-400 bg-slate-100 cursor-not-allowed flex items-center justify-center gap-2"
                                    >
                                        {v.button}
                                    </button>
                                )}
                ...
                <div className="max-w-4xl mx-auto px-6">
                    <div className="grid md:grid-cols-2 gap-12">
                        ...
                        <div className="text-center">
                            ...
                            <p className="text-slate-600">Your images never touch cloud servers. The desktop app ensures complete data sovereignty.</p>
                        </div>
                    </div>
                </div>
            </section>
            ...
            <footer className="py-12 text-center text-slate-400 text-sm">
                <p>Current Release: Stable 1.0.0 | Licensed under MIT</p>
            </footer>
        </div>
    );
}